...
{
    extension(Message<SensorData> message)
    {
        public async Task AckSaveAsync(ILogger logger)
        {
            try
            {
            ...
            {
                logger.LogWarning(ex, "Failed to Ack message for sensor {SensorId}. Message may be redelivered.",
                    message.Data.SensorId);
            }
        }

        public async Task NackSaveAsync(bool requeue, ILogger logger)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                logger.LogWarning(ex,
                    "Failed to Nack message for sensor {SensorId}.", message.Data.SensorId);
            }
        }
    }
